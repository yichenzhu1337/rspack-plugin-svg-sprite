...
│   └── vue-rspack-extract/       # Vue 3 + Rspack extract mode with plainSprite (port 3002)
...
3. **build-examples** — builds all three example apps (depends on test)
...
All examples use `workspace:*` to reference the local plugin. After changing the plugin, run `pnpm build` from root, then test with `pnpm dev:rspack` or `pnpm dev:webpack`.
